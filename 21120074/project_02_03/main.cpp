...
int main(int argc, char* argv[])
{
    ...
    string p_hex, g_hex, x_hex, c1_hex, c2_hex;
   
    getline(input_file, p_hex, '\n');
    getline(input_file, g_hex, '\n');
    getline(input_file, x_hex, '\n');
    getline(input_file, c1_hex, '\n');
    getline(input_file, c2_hex, '\n');

    reverse(p_hex.begin(), p_hex.end());
    reverse(g_hex.begin(), g_hex.end());
    reverse(x_hex.begin(), x_hex.end());
    reverse(c1_hex.begin(), c1_hex.end());
    reverse(c2_hex.begin(), c2_hex.end());

    bitset<MAX_SIZE> p = hex_str_to_bin(p_hex);
    bitset<MAX_SIZE> g = hex_str_to_bin(g_hex);
    bitset<MAX_SIZE> x = hex_str_to_bin(x_hex);
    bitset<MAX_SIZE> c1 = hex_str_to_bin(c1_hex);
    bitset<MAX_SIZE> c2 = hex_str_to_bin(c2_hex);
    ...
    bitset<MAX_SIZE> h = power_mod(g, x, p); //h = g^x mod p => d = (p,g,x) is private key
    string h_str = binary_to_hex_little_endian(h);
    output_file << h_str << endl;
    //cout << h_str << endl;

    bitset<MAX_SIZE> s = power_mod(c1, x, p); // s = c1^x mod p

    // to calculate s^-1 => s.s^-1 = 1 (mod p) => s.s^-1 + a.p = 1 => use bezout ax + by = gcd(x,y) 
    bitset<MAX_SIZE> inverse_s = inverse_mod(s, p);
    bitset<MAX_SIZE> m = mul_mod(c2, inverse_s, p); // m = c2.s^-1 mod p
